...
`ggnet2` does not support all of the graph plotting options provided by the  [`igraph`][igraph] and [`network`][network] packages, and there are a few things that it does not do very well, or not at all. Here is a non-exhaustive list of things that `ggnet2` does not handle, or handles badly:
